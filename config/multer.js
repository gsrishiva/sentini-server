...
const storageS3Option = multerS3({
    s3: new aws.S3({
        ...
        secretAccessKey: CONFIG.aws.AWS_SECRET_ACCESS_KEY_ID,
    }),
...
module.exports = {
    dest: path.resolve(__dirname, '..', 'uploads/documents'), //samething as storage.destination but works like a default option. So we'll leave it here anyway
    storage: storageTypes[CONFIG.storageOption],
    limits: { // determine the limit capacity of a file or number of files to be retrieved from client side
        fileSize: 2 * 1024 * 1024 * 1024 * 1024 * 1024, // 2MB max capacity
    },
